"""Generacion del informe Markdown correspondiente unicamente al avance."""
...
def markdown_table(table: pd.DataFrame) -> str:
    """Convierte un DataFrame pequeno a Markdown sin dependencias adicionales."""
...
def top_ngram_table(frequencies: pd.DataFrame, n: int) -> str:
    """Crea una comparacion lado a lado de los diez primeros n-gramas."""
...
def write_advance_report(
    ...
    """Escribe resultados y explicaciones reproducibles del avance."""
    ...
    conflict_count = int((data.groupby("text")["target"].nunique() > 1).sum())
...
conjunto contiene **{len(data):,} filas** y cinco variables originales:
...
Hay **{counts[0]:,} tweets no relacionados con desastres
({percentages[0]:.2f}%)** y **{counts[1]:,} tweets de desastres reales
({percentages[1]:.2f}%)**. La diferencia es moderada, pero se usa balanceo de
clases en el modelo preliminar. `keyword` tiene {missing_keyword:,} valores
faltantes ({missing_keyword / len(data) * 100:.2f}%) y `location` tiene
{missing_location:,} ({missing_location / len(data) * 100:.2f}%). Ni `text` ni
`target` presentan valores faltantes.

Se encontraron **{duplicated_texts} textos repetidos** y **{conflict_count}
...
Los tweets tienen en promedio {data['words_raw'].mean():.2f} palabras antes de
...
Después de la limpieza quedan en promedio {data['words_clean'].mean():.2f}
tokens por tweet. **{int((data['text_clean'] == '').sum())} tweets quedan sin
...
Se usaron **{metrics['training_rows']:,} tweets para entrenamiento** y
**{metrics['test_rows']:,} para prueba** ({metrics['test_fraction']:.2%}).

| Métrica | Resultado |
| --- | ---: |
| Exactitud | {metrics['accuracy']:.4f} |
| Precisión (desastre) | {metrics['precision']:.4f} |
| Recall (desastre) | {metrics['recall']:.4f} |
| F1 (desastre) | {metrics['f1']:.4f} |
